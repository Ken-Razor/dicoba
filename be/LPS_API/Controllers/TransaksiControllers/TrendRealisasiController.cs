...
{
    public class TrendRealisasiController : ApiController
    {
        public List<ExecutionReportModel> Post([FromBody]ExecutionReportModel erm)
        {
            try
            {
                ...
                DataTable dt = te.GetExecutionReportALL(erm.IDProjectHeader, erm.IsTransformasi);
        ...
        public string get(int erm)
        {
            try
            {              
                TransaksiEksekusi te = new TransaksiEksekusi();
                string result = te.AddReliasasi(erm);
                return result;
            }
            catch (Exception ex)
            {
        ...
        public string Put([FromBody]ExecutionReportModel erm)
        {
            try
            {
                TransaksiEksekusi te = new TransaksiEksekusi();

                var projectheaderid = erm.IDProjectHeader;
                var personnumber = Convert.ToInt32(erm.Keys);
                return te.CheckRealisasi(projectheaderid,personnumber);
            }
            catch (Exception ex)
            {
                throw ex;
