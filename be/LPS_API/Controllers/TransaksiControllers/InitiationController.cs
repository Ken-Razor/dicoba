    ...
    public class InitiationController : ApiController
    {
        ...
        public List<ProjectHeaderModel> Post([FromBody]ProjectHeaderModel ph)
        {
            ...
            foreach (DataRow dr in ti.Get_TransaksiProjectHeader(ph.CreatedBy).Rows)
            {
                ...
                ProjectHeader.StartYear = dr["StartYear"].ToString() != "" ? DateTime.Parse(dr["StartYear"].ToString()).ToString("dd MMMM yyyy", new System.Globalization.CultureInfo("id")):null;
                if (dr["StartMonth"].ToString() != "") ProjectHeader.StartMonth = Convert.ToInt32(dr["StartMonth"]);
                ProjectHeader.EndYear = dr["EndYear"].ToString() != "" ? DateTime.Parse(dr["EndYear"].ToString()).ToString("dd MMMM yyyy", new System.Globalization.CultureInfo("id")) :null;
        ...
        public ProjectHeaderModel Put([FromBody]ProjectHeaderModel ph)
        {
            ...
            List<MasterSLA> sla1 = new List<MasterSLA>();

            List<StatusSLAModel> statusSla = new List<StatusSLAModel>();

            List<GroupModel> ListGroup = new List<GroupModel>();

            List<FileModel> FileModel = new List<FileModel>();
            
            DataSet ds = ti.Get_TransaksiProjectHeader_ByID(ph.IDProjectHeader, ph.CreatedBy);
            DataSet d2 = t2.Get_Master_Sla();
            DataSet d3 = ti.Get_StringRKAT_ByID(ph.IDProjectHeader, ph.CreatedBy);
            ...
            DataTable dt19 = ds.Tables[19];
            DataTable dt20 = ds.Tables[20];
            DataTable dt21 = ds.Tables[21];

            DataTable dt22 = d2.Tables[0];
            DataTable dt23 = d3.Tables[0];
            //----------------------------------//
            ...
            grp1.Id = 0;
            grp1.Description = "All";
            ListGroup.Add(grp1);
            foreach (DataRow dr in md.Get_MasterDepartment().Rows)
            {
                GroupModel grp = new GroupModel();

                if (dr["IDDepartment"].ToString() != "") grp.Id = Convert.ToInt32(dr["IDDepartment"]);
                grp.Description = dr["DepartmentCode"].ToString();
            ...
            #region Status SLA
            statusSla.Add(new StatusSLAModel { IdStatus = "ALL", Description = "All" });
            statusSla.Add(new StatusSLAModel { IdStatus = "Berlaku", Description = "Berlaku" });
            statusSla.Add(new StatusSLAModel { IdStatus = "Tidak Berlaku", Description = "Tidak Berlaku" });
            #endregion


            #region SLA
            foreach (DataRow data in dt22.Rows)
            {
                MasterSLA slaModel = new MasterSLA();
                if (data["NoUrut"].ToString() != "") slaModel.NoUrut = Convert.ToInt32(data["NoUrut"]);
                slaModel.GroupName = data["GroupName"].ToString();
                slaModel.Peraturan = data["Peraturan"].ToString();
                slaModel.StatusSLA = data["StatusSLA"].ToString();
                slaModel.JasaPelayanan = data["JasaPelayanan"].ToString();
                slaModel.Waktu = data["Waktu"].ToString();
                slaModel.DihitungDari = data["DihitungDari"].ToString();
                sla1.Add(slaModel);
            }
            #endregion
            #region KodeRKAT
            if (dt23!=null)
            {
                ...
                {
                    ListProjectHeader.KodeMataAnggaran = dt23.Rows[0]["KodeMataAnggaran"].ToString();
                } 
            }
            #endregion
            ...
            ListProjectHeader.ListSLA = sla1;
            ListProjectHeader.ListGroup = ListGroup;
            ListProjectHeader.ListStatus = statusSla;
