...
{
    public class ExecCRUDApprovalLogController : ApiController
    {
        GlobalFunction GF = new GlobalFunction();
        TxExec TE = new TxExec();
        public EksekusiModel Get(int ProjectID, string Periode, string Persnum)
        {
            DataSet ds = TE.Get_Exec_ApprovalLog(ProjectID, Periode, Persnum);

            //DataTable milestone = ds.Tables[0];
            //DataTable constrain = ds.Tables[1];
            DataTable status = ds.Tables[0];
            DataTable approval = ds.Tables[1];
            DataTable isApproval = ds.Tables[2];
            //DataTable rolegroup = ds.Tables[4];
            DataTable revisidesc = ds.Tables[3];
            DataTable History = ds.Tables[4];
